...
char Usart1ReadBuff[Usart1ReadLen];  //接收数据缓存
char Usart1SendBuff[Usart1SendLen];  //发送数据缓存
...
u8   Usart1ClearIdleIT=0;//清除接收空闲中断使用
...
void USART1_IRQHandler(void)//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)
	{
		...
		BufferManageWrite(&buff_manage_struct2,Usart1ReadBuff,Usart1ReadCnt);//把接收的数据存入缓存
		Usart1ReadCnt=0;
	}
